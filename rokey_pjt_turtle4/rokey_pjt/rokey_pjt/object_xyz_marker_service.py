...
MARKER_TOPIC = '/robot4/detected_objects_marker'
...
class YoloDepthToMap(Node):
    def __init__(self, model, output_dir):
        ...
        # 한 번만 요청할 라벨 목록 및 추적 집합
        self.target_labels = {'bandage', 'coldaewon', 'tylenol', 'cup'}
        self.detected_once = set()
        
        self.cli = self.create_client(ObjectPosition, 'object_position_receiver')
        self.req = ObjectPosition.Request()
        self.get_logger().info(f"Subscribing to topics:\n  RGB: {RGB_TOPIC}\n  Depth: {DEPTH_TOPIC}\n  CameraInfo: {CAMERA_INFO_TOPIC}\n  MarkerPub: {MARKER_TOPIC}")
    ...
    def send_request(self, label: str, x: float, y: float):
        self.req.label = label
        self.req.x = x
        self.req.y = y
        self.get_logger().info(f"[Client] 요청 전송: label='{label}', x={x:.3f}, y={y:.3f}")
        
        future = self.cli.call_async(self.req)
        future.add_done_callback(self.response_callback)  # ✅ 콜백으로 응답 처리

    def response_callback(self, future):
        try:
            response = future.result()
            self.get_logger().info(f"[Client] 응답 수신: success={response.success}, message={response.message}")
        except Exception as e:
            self.get_logger().error(f"[Client] 응답 처리 실패: {e}")
            ...
            for result in results:
                ...
                for box in result.boxes:
                    ...
                    if not np.isnan(map_x):
                        ...
                        # 해당 라벨이 타겟이고 아직 안 보냈을 경우만 요청
                        if label in self.target_labels and label not in self.detected_once:
                            self.send_request(label, map_x, map_y)
                            self.detected_once.add(label)
                    ...
                    cv2.putText(frame, f"{z:.2f}m", (x1, y1 - 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_dict[label], 2)
                    cv2.putText(frame, f"({map_x:.2f}, {map_y:.2f}, {map_z:.2f})", (x1, y1 -5),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_dict[label], 2)
...
def main():
    ...
    parser.add_argument('--model', type=str, required=False, default=YOLO_PATH, help='Path to model file (.pt, .onnx, .engine)')
